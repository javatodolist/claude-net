...
// 当前文章在博客列表中的位置
// route.path 是 URL 解码后的路径；a.url 可能是编码后的路径（含 .html）
// 统一 decode + 去掉 .html 再比较，避免编码不一致导致匹配失败
const currentIndex = computed(() => {
  const p = decodeURIComponent(route.path).replace(/\.html$/, '').replace(/\/$/, '')
  return articles.findIndex(a => {
    const au = decodeURIComponent(a.url).replace(/\.html$/, '').replace(/\/$/, '')
    return au === p
  })
})

function goPrev() {
  // 优先点击 VitePress 原生 pager 链接（文档页/侧边栏顺序）
  const prev = document.querySelector('.pager-link.prev') as HTMLAnchorElement | null
  if (prev) { prev.click(); return }
  // 降级：博客文章列表上一篇
  const idx = currentIndex.value
  if (idx > 0) router.go(articles[idx - 1].url)
}
...
function goNext() {
  ...
  const idx = currentIndex.value
  if (idx >= 0 && idx < articles.length - 1) router.go(articles[idx + 1].url)
}
</script>
...
<template>
  <div class="floating-toolbar">
    ...
    <a class="toolbar-btn" href="https://ai80.net/sitemap.xml" target="_blank" title="站点地图">
